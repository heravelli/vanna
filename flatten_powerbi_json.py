...
# COMMAND ----------
# Flatten Reports with all metadata
...
print(f"Total Reports extracted: {reports_df.count()}")

# COMMAND ----------
# Flatten Datasets with datasource usage
...
# COMMAND ----------
# Extract datasource usage per dataset
...
).join(datasource_instances, "datasource_id", "left")
...
# COMMAND ----------
# Flatten Tables with source information
...
# COMMAND ----------
# Flatten Columns with full details
...
print(f"Total Columns: {columns_df.count()}")
print(f"Calculated Columns: {calculated_columns.count()}")

# COMMAND ----------
# Flatten Measures with full expressions
...
print(f"Total Measures: {measures_df.count()}")
...
# COMMAND ----------
# Aggregate measures per dataset with expressions
...
exact_match_similarity = np.zeros((len(pdf), len(pdf)))
for i in range(len(pdf)):
    for j in range(i, len(pdf)):
        col_match = 1.0 if pdf.iloc[i]['column_schema_hash'] == pdf.iloc[j]['column_schema_hash'] else 0.0
        measure_match = 1.0 if pdf.iloc[i]['measure_schema_hash'] == pdf.iloc[j]['measure_schema_hash'] else 0.0
            ...
            for j in similar_indices:
                ...
                if pair_id not in processed_pairs:
                    ...
                    consolidation_candidates.append({
                        ...
                        'report1_columns': pdf.iloc[i]['total_columns'],
                        'report1_measures': pdf.iloc[i]['total_measures'],
                        'report1_tables': pdf.iloc[i]['total_tables'],
                        'report1_created_by': pdf.iloc[i]['created_by'],
                        'report1_modified_date': pdf.iloc[i]['modified_date'],
                        ...
                        'report2_columns': pdf.iloc[j]['total_columns'],
                        'report2_measures': pdf.iloc[j]['total_measures'],
                        'report2_tables': pdf.iloc[j]['total_tables'],
                        'report2_created_by': pdf.iloc[j]['created_by'],
                        'report2_modified_date': pdf.iloc[j]['modified_date'],
                        ...
                        'column_diff': abs(pdf.iloc[i]['total_columns'] - pdf.iloc[j]['total_columns']),
                        'measure_diff': abs(pdf.iloc[i]['total_measures'] - pdf.iloc[j]['total_measures']),
