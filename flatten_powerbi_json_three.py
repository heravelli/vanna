...
# COMMAND ----------
# Extract Datasource Instances with flexible connection details handling
datasource_instances_raw = df.select(
    ...
    col("ds.connectionDetails").alias("connection_details"),
    col("ds.gatewayId").alias("gateway_id")
).filter(col("datasource_id").isNotNull())

# Convert connectionDetails JSON to string for flexible handling
from pyspark.sql.functions import to_json, from_json, schema_of_json

if datasource_instances_raw.count() > 0:
    # Get all possible fields from connectionDetails
    # Convert to JSON string for text-based similarity
    datasource_instances = datasource_instances_raw.withColumn(
        "connection_details_json",
        to_json(col("connection_details"))
    )
    
    # Try to extract common fields if they exist (server, database, url, path, etc.)
    datasource_instances = datasource_instances.withColumn(
        "server",
        when(col("connection_details.server").isNotNull(), col("connection_details.server"))
        .otherwise(lit(""))
    ).withColumn(
        "database", 
        when(col("connection_details.database").isNotNull(), col("connection_details.database"))
        .otherwise(lit(""))
    ).withColumn(
        "url",
        when(col("connection_details.url").isNotNull(), col("connection_details.url"))
        .otherwise(lit(""))
    ).withColumn(
        "path",
        when(col("connection_details.path").isNotNull(), col("connection_details.path"))
        .otherwise(lit(""))
    ).withColumn(
        "account",
        when(col("connection_details.account").isNotNull(), col("connection_details.account"))
        .otherwise(lit(""))
    )
    
    # Create flexible connection string from all available fields
    datasource_instances = datasource_instances.withColumn(
        "connection_string",
        concat_ws(":",
            col("datasource_type"),
            coalesce(col("server"), lit("")),
            coalesce(col("database"), lit("")),
            coalesce(col("url"), lit("")),
            coalesce(col("path"), lit("")),
            coalesce(col("account"), lit(""))
        )
    )
    
    # Also keep the full JSON for detailed analysis
    datasource_instances = datasource_instances.select(
        "datasource_id",
        "datasource_type",
        "server",
        "database",
    ...
    print("WARNING: No datasource instances found in the data.")
    schema = StructType([
        ...
        StructField("connection_details_json", StringType(), True),
        StructField("gateway_id", StringType(), True)
    ])
    datasource_instances = spark.createDataFrame([], schema)
